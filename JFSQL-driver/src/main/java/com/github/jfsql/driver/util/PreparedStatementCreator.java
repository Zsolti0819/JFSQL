...
public class PreparedStatementCreator {
    ...
    public DeleteWrapper getPreparedDeleteStatement(final DeleteWrapper statement, final JfsqlPreparedStatement preparedStatement) {
        final String tableName = statement.getTableName();
        final List<String> whereColumns = statement.getWhereColumns();
        final List<String> whereValues = replacePlaceholders(preparedStatement, whereColumns, statement.getWhereValues(), 0);
    ...
    public InsertWrapper getPreparedInsertStatement(final InsertWrapper statement, final JfsqlPreparedStatement preparedStatement) throws SQLException {
        final String tableName = statement.getTableName();
        ...
        for (int i = 0; i < statement.getValues().size(); i++) {
            final List<String> values = replacePlaceholders(preparedStatement, columns, new ArrayList<>(statement.getValues().get(i)), 0);
            listOfValueLists.add(values);
        }
        return new InsertStatement(tableName, columns, listOfValueLists);
    }

    public SelectWrapper getPreparedSelectStatement(final SelectWrapper statement, final JfsqlPreparedStatement preparedStatement) {
        final String tableName = statement.getTableName();
        ...
        final List<String> symbols = statement.getSymbols();
        final List<String> binaryOperators = statement.getBinaryOperators();
        return new SelectStatement(tableName, joinTableNames, joinTypes, columns, listOfJoinColumns, whereColumns, whereValues, symbols, binaryOperators);
    }

    public UpdateWrapper getPreparedUpdateStatement(final UpdateWrapper statement, final JfsqlPreparedStatement preparedStatement) {
        final String tableName = statement.getTableName();
        final List<String> columns = statement.getColumns();
        final List<String> values = replacePlaceholders(preparedStatement, columns, statement.getWhereValues(), 0);
        final List<String> whereColumns = statement.getWhereColumns();
        final List<String> whereValues = replacePlaceholders(preparedStatement, whereColumns, statement.getWhereValues(), columns.size());
        final List<String> symbols = statement.getSymbols();
        final List<String> binaryOperators = statement.getBinaryOperators();
        return new UpdateStatement(tableName, columns, values, whereColumns, whereValues, symbols, binaryOperators);
    }

    private List<String> replacePlaceholders(final JfsqlPreparedStatement preparedStatement, final List<String> columns, final List<String> values, final int offset) {
