...
public class DatabaseManagerFactory {
    ...
    public DatabaseManager createDatabaseManager(final PropertiesReader propertiesReader, final String URL,
        final SemanticValidator semanticValidator, final Reader reader,
        final Writer writer) throws SQLException {
        final TransactionVersioning type = propertiesReader.getTransactionVersioning();
        switch (type) {
            case JGIT:
                return new JGitDatabaseManagerImpl(URL, semanticValidator, reader, writer);
            case DEFAULT:
                return new DefaultDatabaseManagerImpl(URL, semanticValidator, reader, writer);
            default:
